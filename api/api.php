...
if (!$isLoggedIn) {
    ...
    echo json_encode([
        ...
        "preview_count" => null,
        "isFinished"    => false,
        "is_new"        => false // 👑 鐵律：未登入模式下固定為 false， class="new" 這格絕對看不到 NEW
...
/* =========================================================================
   最高鐵律防線二：[daily_progress 三組獨立計數與跨天自動重置機制]
   ========================================================================= */
$db_progress = [];
...
if ($is_finished_now === true) {
    ...
    if ($action !== null) {
        if ($action === 'correct') {
            // ➔ 任務結束後點擊 correct：隨機抽取「舊字庫」（歷史紀錄）複習
            ...
            $word_data = !empty($res) ? $res[0] : null;
            if ($word_data) {
                // 完工後盲刷隨機抽到新字，因為不寫入資料庫，僅在記憶體中模擬初始等級
                $word_data['learning_level'] = 1;
                $word_data['preview_count'] = 0;
            }
        }
    }

    // 完工狀態下若前端剛載入或未傳送具體 action，預設提供一筆舊字紀錄盲刷
    if (!$word_data) {
        $res = $currentDB->q("SELECT lr.*, $sql_fields FROM `learning_records` lr JOIN `$table` t ON lr.word_id = t.id $sql_join WHERE lr.learner_id = '$learner_id' AND lr.type = '$do' ORDER BY RAND() LIMIT 1");
        $word_data = !empty($res) ? $res[0] : null;
    }
    ...
    if (!$word_data) {
        echo json_encode(["status" => "empty", "message" => "特訓字庫已無可用卡片。"], JSON_UNESCAPED_UNICODE);
        exit;
    }

    // 💡 遵照最高鐵律：完成課題模式下，絕對不對資料庫進行任何 save 寫入，直接包裝輸出
    echo json_encode([
        "status"        => "success",
        ...
        "translation"   => $word_data['translation'] ?? "",
        "audio"         => $word_data['audio_url']   ?? "", 
        "level"         => $word_data['learning_level'] ?? 1,
        "preview_count" => $word_data['preview_count']  ?? 1,
        "isFinished"    => true,
        "is_new"        => false // 👑 鐵律：完成課題模式下固定為 false，class="new" 這格絕對看不到 NEW
    ], JSON_UNESCAPED_UNICODE);
    exit; // 任務完成延伸特訓在此直接中斷
}
...
if ($has_due_words) {
    ...
    if ($under_lv5 >= 200) {
        ...
        // 未滿 200 字，放行結算今日舊字正確率，以此動態核發今日新字上限額度
...
if ($has_due_words) {
    ...
    $word_data = !empty($res) ? $res : null;
    $is_this_turn_new = false; // 舊字複習不亮 NEW
} else {
    /* ----- 遞補軌道：抽全新字（智慧防重複防防線） ----- */
    ...
    if ($new_count < $new_limit) {
        ...
        if ($word_data) {
            $is_this_turn_new = true; // 👑 一般模式下第一次遇到新單字！確立標籤
            $word_data['learning_level'] = 1;
            $word_data['preview_count'] = 0; // 還沒有點擊作答前，在基礎資料包內先標註出現 0 次
        }
    }
}
...
echo json_encode([
    ...
    "is_new"        => $is_this_turn_new  // 💡 正確對接：新字則回傳 true，前端依此將 'NEW' 填入 class="new" 容器內
], JSON_UNESCAPED_UNICODE);
